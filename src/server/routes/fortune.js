...
const { getTodayFortune, getWeekFortune } = require('../../engine/todayFortune');
...
router.get('/today-fortune', requireAuth, (req, res) => {
  ...
  try {
    ...
    const fortune = getTodayFortune(birth);
    const week = getWeekFortune(birth);
    res.json({ fortune, week });
  } catch (e) {
