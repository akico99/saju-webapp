...
const NOBLE = {
  '천을귀인': {
    title: '천을귀인 — 도움을 떠올리는 표지',
    core: '전통 명리에서 보호와 도움을 상징하는 길신으로 읽습니다. 실제 인물의 등장이나 위기 극복을 보장하는 표지는 아닙니다.',
    tip: '도움을 기대하기보다, 필요할 때 요청하고 받은 도움을 기록해 보세요.'
  },
  '문창귀인': {
    title: '문창귀인 — 글과 배움을 살피는 표지',
    core: '전통 명리에서 문서·학문·표현을 상징하는 길신으로 읽습니다. 시험·자격·글쓰기·기획의 결과를 보장하지는 않습니다.',
    tip: '말보다 글로, 즉흥보다 정리해서 내놓을 때 이 기운이 삽니다. 중요한 제안은 문서로 남기세요.'
  },
  '암록': {
    title: '암록 — 드러나지 않는 자원을 살피는 표지',
    core: '전통 명리에서 드러나지 않은 도움이나 자원을 상징하는 표지로 읽습니다. 수입·부·후원자를 보장하거나 예측하지는 않습니다.',
    tip: '막연한 기대보다 현재의 수입·지출과 관계를 차분히 점검해 보세요.'
  },
  '장성살': {
    title: '장성살 — 책임과 주도성을 살피는 표지',
    core: '전통 명리에서 주도성과 책임을 상징하는 신살로 읽습니다. 실제 리더 자리·성공·인정을 보장한다는 뜻은 아닙니다.',
    tip: '작은 일 하나의 책임 범위와 협력 방식을 정해 보세요.'
  },
  '반안살': {
    title: '반안살 — 자리와 안정감을 살피는 표지',
    core: '말안장에 오른 모습에서 유래해 전통 명리에서 자리와 안정을 상징하는 신살로 읽습니다. 지위·체면·대접을 보장하거나 예측하지는 않습니다.',
    tip: '격을 지키려 하기보다, 맡은 역할과 기준을 분명히 해 보세요.'
  }
};
...
function readNoble(r) {
  ...
  for (const key of pillars) {
    ...
    for (const raw of p.shinsals) {
      ...
      // 같은 기둥에서 계산 기준만 달리해 반복된 신살은 한 번만 보여준다.
      const id = `${key}:${name}`;
